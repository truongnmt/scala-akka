...
class BasicSpec extends TestKit(ActorSystem("BasicSpec"))
  ...
  "A simple actor" should {
    "send back the same message" in {
      ...
      expectMsg(message) // akka.test.single-expect-default = 3 seconds (default timeout, configurable)
    }
  }
...
// recommend to create companion object to store all the methods, values
