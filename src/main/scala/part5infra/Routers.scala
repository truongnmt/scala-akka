...
/*
  ...
  - smallest mailbox
  - broadcast
  - scatter-gather first: send the message to all the routees, and wait for the first response, and all other responses are discarded
  - tail-chopping: send the message to routees one by one in a predefined order, with a small delay between each message.
    If the first routee doesn't response within that delay, the message is sent to the next routee. Once a response is
    received, all other responses are discarded.
  - consistent hashing: the same hash code will be sent to the same routee
